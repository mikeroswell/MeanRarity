...
purrr::map(c("lnorm", "gamma", "invgamma"), function(dist){
  # quartz(title = dist, width = 9, height = 7)
  myp<-err_plot_data %>% 
    ...
    # ggplot2::scale_y_continuous(limits = c(max(min(this_error*100), -200), min(max(this_error*100), 200)) ) +
    ggplot2::scale_y_continuous(
      breaks = scales::breaks_extended(5)
      , labels = function(x){
          naked =  exp(abs(x))
          myt = ifelse(naked>1, naked
                       , ifelse(isTRUE(all.equal(naked, 1))| x ==0, 1
                       , scales::label_math()))   
          return(myt)
        }
                                ) +
    ...
    #                             , n.breaks = 5
    #                             ) +
    # ggbreak::scale_y_break(c(-1, 1), expand =F) + 
...
purrr::map(c("lnorm", "gamma", "invgamma"), function(dist){
  ...
  myp<-err_plot_data %>% 
    ...
    # ggplot2::scale_y_continuous(limits = c(max(min(this_error*100), -200), min(max(this_error*100), 200)) ) +
    ggplot2::scale_y_continuous(trans = "mult"
                                ) +
    ggbreak::scale_y_cut(c(-1,1)) +
    # ggplot2::scale_y_continuous(trans = scales::pseudo_log_trans(sigma = 1
    #                                                              , base = exp(1))
    #                             , n.breaks = 5
    #                             ) +
