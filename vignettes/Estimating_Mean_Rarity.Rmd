...
functions for generating and sampling abundance vectors and 
generating plots. This vignette is not a comprehensive
...
summary of those data, `beeAbunds`. The function `radplot()` takes those
...
Visualizing the mean rarity by finding a physical balance point helps clarify 
how rarity is scaled by different means, but always weighted by the abundance of 
individuals. The function `rarity_plot` generates the figures that can also be 
generated interactively in the shiny app 
...
diversities compute mean rarity, and is simpler than 
