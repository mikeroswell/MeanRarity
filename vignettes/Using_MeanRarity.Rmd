...
To explore this, we can first simulate species abundance distributions using the
...
minimum, 1, for any richness value [@Chao2019]. Thus, in each simulated assemblage, we know richness to be 100, Hill-Simpson takes one of three values given by the evenness paramters, and other Hill diversities depend on the distributional assumption (gamma or lognormal). 

Intuitively, sampling variability in Hill diversity must depend jointly on sample size, the species abundance distribution, and the scaling exponent for Hill diversity. 

<-- Figures showinng bias for sample diveristy and Chao/Jost asymptotic estimators -->



Much less explored is the variability of both sample diversity, and of asymptotic diversity estimates, under random sampling expectations. 

```{r eveness makes Hill-Simpson hard}
