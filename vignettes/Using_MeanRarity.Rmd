...
This R package provides functions for three purposes: (1) the computation of
Hill diversities, using the rarity parameterization, (2) the visualization of
mean rarity, rank abundance distributions, and Hill diversity profiles, and (3)
the simulation of species abundance distributions, where the diversity of each
resulting assemblage is a simulation parameter *per se*, rather than a
downstream result. The package is availabe online on Github (see Data
accessibility). An interactive shiny application for building "balance plots" is
also available online (https://mean-rarity.shinyapps.io/rshiny_app1/).
