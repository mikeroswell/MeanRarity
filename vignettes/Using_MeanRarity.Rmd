...
remain under-explored. To facilitate such intuition, we developed the R package
**MeanRarity**, which enables estimating and visualizing Hill diversity (aka
"Hill numbers") in terms of the average species "rarity." Rarity is defined as
the reciprocal of relative abundance.
...
equivalent to `vegan::renyi(x, scale = 1-l, hill = T)`, `entropart::Diversity(x, 1-l, Correction = "None"))`, or the `qD` output from `iNEXT::estimateD(x)` where
`order = 1-l`.
...
true diversity), hereafter "rmsle." <!-- give JD's 1% with compounding
definition in figure caption! --> For bias, we plotted the log ratio between the
Hill diversity estimate and its target, and for variability we plotted and the
standard deviation of the log Hill diversity (a unitless measure of
