...
unite variants of the most popular diversity metrics with a single equation
...
(aka "Hill numbers") in terms of the average species "rarity." Rarity is defined
as the reciprocal of relative abundance.
...
[e.g., @Chao2014b; @Haegeman2013; @Tuomisto2010]. Another interpretation
...
diversity, and Hill-Simpson diversity in purple, red, and green respectively).
...
given sample from such as species abundance distribution is, of course, not
...
**sads** (CITE) allow users to specify continuous parametric probability
...
abundances for an assemblage with infinite abundance. To take a finite sample,
...
diversity as sample size approaches infinity. <!-- the section above could be
tightened. A key issue is that the paragraphs above make it clear what `fit_SAD`
does but not as much what it is for. A central point is that one generates
samples from the distribution given by fit_SAD-->
...
reasonably large (hundreds of individuals) and $\ell < \sim 0.25 $. <!--
...
1. **EntropyEstimation**
  ...
  indices. the function `Hill.z()` computes the exponentiated estimated Renyi
  entropy (a.k.a. Hill diversity) for order $r = 1-l$. For comparison with other 
  Hill diversity estimators, users are referred to **entropart**.
...
1. **Richness** 
  ...
  generalized to Hill diversities other than richness.
<!-- search for others? -->
