...
tog <- allseveral %>% 
  dplyr::mutate(sm = paste(standard, Assemblage, sep=""))
...
leftPlot <- togn %>% 
    ...
    ggplot2::ggplot(aes(Assemblage, qD, color = Assemblage, shape = Assemblage)) +
...
rightPlot <- togn %>% 
  dplyr::filter(methlab == "asymptotic") %>% 
  ggplot2::ggplot(ggplot2::aes(Assemblage, qD, color = Assemblage, shape = Assemblage)) +
