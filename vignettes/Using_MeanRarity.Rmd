...
err_plot_data %>% 
  ...
  dplyr::mutate(err_type = factor(err_type
                                  ...
                                  , labels = c("Average Error \n\n(back-transformed rmsle)", "Variability \n\n(back-\ntransformed \nsdlog)", "Proportional Bias \n\n(back-transformed \nlog accuracy ratio)")
                                  )
                , est = if_else(est == "estimator", "asymptotic"
                              , if_else(est == "naive", "naïve", "sample"))
                , exp_err = exp(this_error)
                ) %>% 
  dplyr::filter(distribution == "lnorm" # "gamma"
                ) %>% 
  ggplot2::ggplot(ggplot2::aes(x = ell
                               , color = evenness)) +
  ggplot2::geom_line(ggplot2::aes(y = exp_err
  ...
  ggplot2::scale_color_manual(values = hcl.colors(4, palette = "SunsetDark")[1:3]
