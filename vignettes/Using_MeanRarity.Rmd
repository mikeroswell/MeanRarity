...
## Evenness and the variability in Hill diversity

```{r eveness makes Hill-Simpson hard}
evenness<-10^seq(-0.75,-0.35,0.1)
rich <- 100
...
sample_data<-purrr::map_dfr(SADS, function(mySAD){
  purrr::map_dfr(seq(-1.5, 1.5, 0.01), function(ell){
    true_diversity = rarity(ab = mySAD[[3]], l = ell)
    sample_diversity = replicate(300, rarity(sample_infinite(ab = mySAD[[3]], size = SS), l = ell))
    evenness = (mySAD[[2]][3]-1)/(mySAD[[2]][1]-1)
    distribution = mySAD[[1]][1]
    return(data.frame(true_diversity, sample_diversity, evenness, distribution, ell = ell, row.names = NULL))
  })
})

sample_data %>% 
  dplyr::group_by(evenness, distribution, ell) %>% 
  dplyr::summarize(sdlog_diversity = sd(ifelse(sample_diversity == 0, 0, log(sample_diversity)))
            , true_diversity = mean(true_diversity)
            , mean_sample_diversity = mean(sample_diversity)) %>% 
  ...
  ggplot2::geom_smooth(ggplot2::aes(color = evenness, linetype = distribution), se = F ) +
  ggplot2::theme_classic() +
  ggplot2::xlim(c(-1,1)) + 
