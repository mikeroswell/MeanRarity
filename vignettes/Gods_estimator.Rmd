...
ecologists). For SAD 19 we see very large RMSE for species richness. This is because God, but not the other estimators, can distinguish the merely rare from the exceedingly rare, and SAD 19 has wildly rare species (a quarter of species have relative abundance of < 1/ 10 million). When these species are included randomly in a sample, the mean rarity of that sample is very high. Our simulation may be untroubling then, even for proponents of God's estimator. When ecologists sample SADs with hundreds of millions of individuals (e.g. Rhizospheres), they likely do not attempt to quantify richness, given that it cannot be estimated reliably (CITE Haegemen et al. 2013),  and also obtain reads from many more than a few hundred individuals. In this second case, God's estimator is expected to perform adequately:

```{r Gods estimator in the face of extreme rarity}
nreps <- 999
...
developing heuristics to estimate true species rarities might enable enable
...
estimators more commonly used today.
