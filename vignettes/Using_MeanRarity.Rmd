...
reasonably large (hundreds of individuals) and $\ell < \sim 0.25 $. Less
...
well as for the naïve estimator, but when sample diversity is not treated as an
estimator for the full assemblage diversity, it has no "bias" (CITE Smith and
Grassle, Hurlbert, Dauby & Hardy). Evenness and distributional assumption of the
SAD affected the bias for both the näive and asymptotic Hill diversity
estimates. More even communities tended to exhibit lower bias for richness but
higher bias for Hill-Simpson diversity, although this pattern depended to some
degree on both distributional assumption and sampling intensity. For the
asymptotic Hill diversity estimates, bias at very low sample sizes was less than
sample diversity but could still be extreme. However, this bias evaporated
quickly for both sample and asymptotic Hill diversity estimates when the scaling
exponent $\ell << 1 $.
...
# pdf("/Users/mikeroswell/Dropbox/bias_variability_figs_condensed.pdf")
# total error for sample diversity


err_plot_data %>% 
  mutate(naive_sdlog = sample_sdlog) %>% 
  pivot_longer(c(5:12, 16)
               , names_to = c("est", "err_type")
               , names_pattern = "(.*)_(.*)"
               , values_to = "this_error"
  ) %>% 
  mutate(col.col = paste0(est, evenness)) %>% 
  ggplot2::ggplot(ggplot2::aes(x = ell
                               , color = col.col
                               , shape = distribution)) +
  ggplot2::geom_line(ggplot2::aes(y = this_error
    , linetype = distribution), size = 0.8) +
  ggplot2::theme_classic() +
  ggplot2::xlim(c(-1,1)) +
  ggplot2::scale_color_manual(values = c(hcl.colors(4, palette = "BluGrn")[1:3]
                                         , hcl.colors(4, palette = "RedBlue")[1:3]
                                         , hcl.colors(4, palette = "SunsetDar")[1:3])) +
  ...
  #function(x){sapply(x, function(y){paste(y, " individuals")})}) +
  ggplot2::labs(x = "scaling exponent \U2113"
                # , y = "sample diversity root mean squared log error"
                ) +
  # ggplot2::scale_y_continuous(limits = c(-0, 1.7), expand = c(0,0)) +
...
err_plot_data %>%
  ggplot2::ggplot(ggplot2::aes(x = ell
                               , color = evenness
                               , shape = distribution)) +
  ggplot2::geom_line(ggplot2::aes(y = sample_rmsle
    , color = evenness, linetype = distribution), size = 0.8) +
  ggplot2::theme_classic() +
  ggplot2::xlim(c(-1,1)) +
  ggplot2::scale_color_manual(values = hcl.colors(4, palette = "BluGrn")[1:3]) +
...
# treating the sample diversity as a naive diversity estimator
# total error for sample diversity
err_plot_data %>%
  ggplot2::ggplot(ggplot2::aes(x = ell
                               , color = evenness
                               , shape = distribution)) +
  ggplot2::geom_line(ggplot2::aes(y =  naive_rmsle
    , color = evenness, linetype = distribution), size = 0.8) +
  ggplot2::theme_classic() +
  ggplot2::xlim(c(-1,1)) +
  ggplot2::scale_color_manual(values = hcl.colors(4, palette = "Red-Blue")[1:3]) +
  ggplot2::facet_wrap(~sample_size
                      , labeller = "label_both") +
  #function(x){sapply(x, function(y){paste(y, " individuals")})}) +
  ggplot2::labs(x = "scaling exponent \U2113"
                , y = "naïve estimator root mean squared log error") +
  ggplot2::scale_y_continuous(limits = c(-0, 1.7), expand = c(0,0)) +
  ggplot2::geom_hline(yintercept = 0, size = 0.2)

# total error for asymptotic diversity
err_plot_data %>%
  ggplot2::ggplot(ggplot2::aes(x = ell
                               , color = evenness
                               , shape = distribution)) +
  ...
  ggplot2::scale_color_manual(values = 
                                hcl.colors(4, palette = "SunsetDark")[1:3]) +
  ggplot2::facet_wrap(~sample_size
                      , labeller = "label_both") +  
  #function(x){sapply(x, function(y){paste(y, " individuals")})}) +
  ggplot2::labs(x = "scaling exponent \U2113"
                , y = "asymptotic estimator root mean squared log error") +
  ggplot2::scale_y_continuous(limits = c(-0, 1.7), expand = c(0,0)) +
  ggplot2::geom_hline(yintercept = 0, size = 0.2)


# sdlog of sample diversity
err_plot_data %>%
  ggplot2::ggplot(ggplot2::aes(x = ell
                               , color = evenness
                               , shape = distribution)) +
  ...
  ggplot2::theme_classic() +
  ggplot2::xlim(c(-1,1)) +
  ggplot2::scale_color_manual(values = hcl.colors(4, palette = "RedBlue")[1:3]) +
  ggplot2::facet_wrap(~sample_size
                      , labeller = "label_both") +  
  #function(x){sapply(x, function(y){paste(y, " individuals")})}) +
  ggplot2::labs(x = "scaling exponent \U2113"
                , y = "sd log(sample diversity)") +
  ggplot2::scale_y_continuous(limits = c(-0, 0.6), expand = c(0,0)) +
  ggplot2::geom_hline(yintercept = 0, size = 0.2)

# sdlog asymptotic diversity
err_plot_data %>%
  ggplot2::ggplot(ggplot2::aes(x = ell
                               , color = evenness
                               , shape = distribution)) +
  ...
  #function(x){sapply(x, function(y){paste(y, " individuals")})}) +
  ggplot2::labs(x = "scaling exponent \U2113"
                , y = "sd log(asymptotic estimator)") +
  ggplot2::scale_y_continuous(limits = c(-0, 0.6), expand = c(0,0)) +
  ggplot2::geom_hline(yintercept = 0, size = 0.2)

# log ratio bias in sample diversity... w.r.t. expected value
err_plot_data %>%
  ggplot2::ggplot(ggplot2::aes(x = ell
                               , color = evenness
                               , shape = distribution)) +
  ...
  #function(x){sapply(x, function(y){paste(y, " individuals")})}) +
  ggplot2::labs(x = "scaling exponent \U2113"
                , y = "mean log(observed/expected sample diversity)") +
  ggplot2::scale_y_continuous(limits = c(-1.8, 0.3)) +
  ggplot2::geom_hline(yintercept = 0, size = 0.2)

# bias for naïve estimator
err_plot_data %>%
  ggplot2::ggplot(ggplot2::aes(x = ell
                               , color = evenness
                               , shape = distribution)) +
  ...
  #function(x){sapply(x, function(y){paste(y, " individuals")})}) +
  ggplot2::labs(x = "scaling exponent \U2113"
                , y = "mean log(observed/true diversity)") +
  ggplot2::scale_y_continuous(limits = c(-1.8, 0.3)) +
  ggplot2::geom_hline(yintercept = 0, size = 0.2)
...
err_plot_data %>%
  ...
  ggplot2::labs(x = "scaling exponent \U2113", y = "mean log(estimated asymptotic/true diversity)") +
  ggplot2::scale_y_continuous(limits = c(-1.8, 0.3)) +
  ggplot2::geom_hline(yintercept = 0, size = 0.2)
...
occam <- err_plot_data %>% 
  group_by(evenness, distribution, ell, sample_size) %>% 
  summarize(total = estimator_rmsle > naive_rmsle
            , bias = abs(estimator_bias) > abs(naive_bias)
            , variability = estimator_sdlog > sample_sdlog)


occam %>% 
  filter(total %in% c(TRUE, FALSE)) %>% 
  ggplot(aes(total, ell, color = as.factor(sample_size), shape = distribution ))+
  geom_jitter(height = 0, width = 0.4)+
  scale_color_manual(values = hcl.colors(4, "Plasma")) +
  facet_wrap(~evenness)+
  labs(x = "naive estimator beats Chao (rmsle)")+
  ylim(c(-1, 1))+
  theme_classic()


occam %>% 
  filter(bias %in% c(TRUE, FALSE)) %>% 
  ggplot(aes(bias, ell, color = as.factor(sample_size), shape = distribution ))+
  geom_jitter(height = 0, width = 0.4)+
  scale_color_manual(values = hcl.colors(4, "Plasma")) +
  facet_wrap(~evenness)+
  labs(x = "naive estimator beats Chao (log accuracy ratio)")+
  ylim(c(-1, 1))+
  theme_classic()

occam %>% 
  filter(variability %in% c(TRUE, FALSE)) %>% 
  ggplot(aes(variability, ell, color = as.factor(sample_size), shape = distribution ))+
  geom_jitter(height = 0, width = 0.4)+
  scale_color_manual(values = hcl.colors(4, "Plasma")) +
  facet_wrap(~evenness)+
  labs(x = "naive estimator beats Chao (sdlog)")+
  ylim(c(-1, 1))+
  theme_classic()
