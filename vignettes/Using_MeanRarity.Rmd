...
rmsle<- function(x, true_x = mean(log(x))){
  sqrt(mean((log(x)-true_x)^2))
}

plot_data<-sample_data %>%
  dplyr::group_by(evenness, distribution, ell, SS) %>%
  dplyr::summarize(sample_sdlog = sd(ifelse(sample_diversity == 0
                                      , 0, log(sample_diversity)))
                   , asymptotic_sdlog = sd(ifelse(estimated_diversity == 0
                                            , 0, log(estimated_diversity)))
                   , sample_bias = mean(log(sample_diversity)- mean(log(sample_diversity)))
                   , estimator_bias = mean(log(estimated_diversity)-log(true_diversity))
                   , sample_rmsle = rmsle(sample_diversity)
                   , estimator_rmsle = rmsle(estimated_diversity, log(true_diversity))
...
plot_data %>%
  ...
  ggplot2::theme_classic() +
  ggplot2::xlim(c(-1,1)) +
   # ggplot2::scale_color_manual(values = c("#007E78", "#65BB8C", "#D4F3A3")) +
  ggplot2::scale_color_manual(values = hcl.colors(5, palette = "Emrld")[2:4]) +
  # ggplot2::scale_color_manual(values = c("#006ddb","#490092", "#b66dff"))+
  # ggplot2::scale_color_viridis_d(option = "plasma") +
#   ggplot2::scale_color_manual(values = c("#fecc5c",
# "#fd8d3c",
# "#e31a1c")) +
  # ggplot2::scale_color_brewer(palette = "BrBG") +
  ggplot2::facet_wrap(~sample_size
                      , labeller = "label_both") +  
  #function(x){sapply(x, function(y){paste(y, " individuals")})}) +
  ggplot2::labs(x = "scaling exponent \U2113"
                , y = "scaled average (sample - true diversity)") +
...
plot_data %>%
  ...
  ggplot2::theme_classic() +
  ggplot2::xlim(c(-1,1)) +
  ggplot2::scale_color_brewer(palette = "PuRd") +
  ggplot2::facet_wrap(~sample_size
                      , labeller = "label_both") +  
  #function(x){sapply(x, function(y){paste(y, " individuals")})}) +
  ggplot2::labs(x = "scaling exponent \U2113"
...
plot_data %>%
  ggplot2::ggplot(ggplot2::aes(x = ell
                               ...
                               , shape = distribution)) +
  ggplot2::geom_line(ggplot2::aes(
    y = (mean_asymptotic-true_diversity)/true_diversity
    , color = evenness, linetype = distribution), size = 0.8) +
  ggplot2::theme_classic() +
  ggplot2::xlim(c(-1,1)) +
  ggplot2::scale_color_brewer(palette = "Set2") +
