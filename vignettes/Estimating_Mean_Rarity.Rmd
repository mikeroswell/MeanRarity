...
species diversity" Oikos doi. Thus, the examples in the vignette are ordered according to the figures in the
article.
...
site_pro %>%  ggplot(aes(ell, d, color = site)) +
    ...
    scale_color_brewer(palette = "Dark2")
```
Fig 4 uses the R package `iNEXT`, which is not imported by `MeanRarity`. 
```{r Figure 4: diversity estimates with CI, fig.height = 4.6, fig.width = 7, eval = FALSE}
# make sure eval = T eventually and delete this comment
tictoc::tic() #time this
...
minc <- min(by_effort %>% group_by(site) %>% dplyr::summarize(SC = max(SC)) %>% pull(SC)) # coverage of sample with lowest


tog <- allseveral %>% mutate(sm=paste(standard, site, sep=""))

inds <- data.frame("order" = c(0, 1, 2), divind = factor(c("richness"
                                                           , "Hill-Shannon"
                                                           , "Hill-Simpson")
                                                 , levels = c("richness"
                                                              , "Hill-Shannon"
