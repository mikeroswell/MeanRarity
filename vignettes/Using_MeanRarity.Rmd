...
1. `iNEXT`
  ...
  are highly integrated, generating multiple outputs and computing bootstrapped variance
  estimates by default, whereas in `MeanRarity` most exported functions are
  comparatively simple. `MeanRarity` `MeanRarity` provides tools for simulating species
...
1. `SpadeR`
  ...
  version of`SpadeR:::Chao_Hill_abu()` is a n exported function in `MeanRarity`)
1. `vegan`
1. `sads`
1. `mobsim`
